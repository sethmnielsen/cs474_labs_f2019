...
good_characters = string.printable
good_characters = good_characters[:64]
good_characters += "()\',-.;?: \t\n\r"
...
def evaluate(decoder=None, prime_str='A', predict_len=200, temperature=0.8):
    ...
    for k in range(predict_len):
        ...
        char_choice = good_characters[select_ind]
...
def main_loop(decoder: RNN):
    ...
    for epoch in range(1, n_epochs + 1):
        inp, target, chunk = random_training_set()
        loss_ = train(inp, target, chunk, decoder, decoder_optimizer, criterion)
        loss_avg += loss_
...
def produce_samples(decoder: RNN):
    for i in range(10):
        start_strings = ["Jedi", "Luke", "Use the Force", "Lord Vader", "What", " Go", " lo", " ra", "fo", "Fi", "We", " je", " li"]
        start = i
