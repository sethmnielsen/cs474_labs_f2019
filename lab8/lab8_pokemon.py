...
# %%
samples = torch.randn(5, 3, 128, 128) # batch size x channels x width x height
D(samples)
...
# drive.mount('/gdrive')
# get_ipython().run_line_magic('cd', "'/gdrive/My Drive/School/DeepLearning'")

img_size = 64
batch_size = 32
lr = 0.0002

dataset = datasets.ImageFolder(root='/home/seth/Downloads/pokemon-images-dataset',
...
def train_gan(G, D, G_optimizer, D_optimizer, BCE_loss, train_loader, fixed_z_, train_epoch, batch_size):
    num_iter = 0
    collect_x_gen = []
...
# %%
train_epoch = 50
list_of_vars = train_gan(G, D, G_optimizer, D_optimizer, BCE_loss, train_loader, fixed_z_, train_epoch, batch_size)
...
dataset_poke = datasets.ImageFolder(root='./pokemon-images-dataset',
                           transform=transforms.Compose([
                               transforms.Resize(img_size),
                               transforms.CenterCrop(img_size),
                               transforms.ToTensor(),
...
##### Create the dataloader #####
poke_loader = DataLoader(dataset_poke,
                          batch_size=batch_size,
                          pin_memory=True)
...
Gpoke = Generator(128)
Dpoke = Discriminator(128)
Gpoke.weight_init(mean=0.0, std=0.02)
Dpoke.weight_init(mean=0.0, std=0.02)
Gpoke = Gpoke.cuda()
Dpoke = Dpoke.cuda()

# Binary Cross Entropy loss
BCE_loss_poke = nn.BCELoss()

# Adam optimizer
G_optimizer_poke = optim.Adam(Gpoke.parameters(), lr=lr, betas=(0.5, 0.999))
D_optimizer_poke = optim.Adam(Dpoke.parameters(), lr=lr, betas=(0.5, 0.999))
...
# %%
list_of_vars_poke = train_gan(Gpoke, Dpoke, G_optimizer_poke, D_optimizer_poke,
                         BCE_loss_poke, poke_loader, fixed_z_, train_epoch)
collect_x_gen_poke, Gpoke, Dpoke, G_optimizer_poke, D_optimizer_poke, BCE_loss_poke = list_of_vars_poke


# %%
for x_gen in collect_x_gen_poke:
    show_imgs(x_gen)
